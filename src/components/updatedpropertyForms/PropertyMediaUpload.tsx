...
const PropertyMediaUpload: React.FC<PropertyMediaUploadProps> = ({
  ...
  const handlePhotoUpload = useCallback(async (event: ChangeEvent<HTMLInputElement>, category: string) => {
    ...
    try {
      ...
      const response = await axios.post('https://rentamigotech.onrender.com/api/property-media/upload', formData);
  ...
  const handleDeletePhoto = useCallback(async (category: string, photoUrl: string) => {
    ...
    if (propertyId) {
      try {
        ...
        await axios.delete(`https://rentamigotech.onrender.com/api/property-media/${propertyType}/${propertyId}/${encodedUrl}`);
      } catch (error) {
