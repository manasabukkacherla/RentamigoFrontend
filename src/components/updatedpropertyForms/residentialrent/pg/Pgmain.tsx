...
useEffect(() => {
  const fetchPGDataForEdit = async () => {
    if (routePropertyId) {
      try {
        const response = await axios.get(`https://rentamigotech.onrender.com/api/residential/pgmain/${routePropertyId}`);
...
if (propertyId) {
  // EDIT MODE
  response = await axios.patch(`https://rentamigotech.onrender.com/api/residential/pgmain/${propertyId}`, payload, {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  toast.success('PG listing updated!');
} else {
  // CREATE MODE
  response = await axios.post('https://rentamigotech.onrender.com/api/residential/pgmain', payload, {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
