...
const MapSelector: React.FC<MapSelectorProps> = ({ latitude, longitude, onLocationSelect, initialShowMap = true }) => {
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      script.src = `https://maps.googleapis.com/mapshttps://rentamigotech.onrender.com/api/js?key=${import.meta.env.VITE_GOOGLE_MAPS_API_KEY}&libraries=places,geocoding`
      script.async = true
      script.defer = true
      script.onload = () => {
        if (window.google && window.google.maps) {
